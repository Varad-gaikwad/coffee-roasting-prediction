...
df = pd.read_csv("/Users/varad/Codes/Coffee roasting/coffee_roasting_dataset.csv")
...
model=Sequential([normalizer, Dense(8, activation='relu'), Dense(4, activation='relu'), Dense(1, activation='sigmoid')])
...
time =float(input("Enter roasting time (minutes): "))
prediction = model.predict(np.array([[temperature, time]]))
